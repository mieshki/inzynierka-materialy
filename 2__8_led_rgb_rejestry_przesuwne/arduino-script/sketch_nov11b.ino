...
void setLed(unsigned long mask_int){
  ...
  /*
  Serial.print("mask[0]=");
  Serial.print(mask[0], BIN);

  Serial.print(", mask[1]=");
  Serial.print(mask[1], BIN);

  Serial.print(", mask[2]=");
  Serial.println(mask[2], BIN);
...
void loop() {
  ...
  for(int i = 0; i < 8; i++){
    ...
    mask = random(0, 16777215);
    
    Serial.print("I=");
    Serial.println(i);
    
    mask = (unsigned long)1 << i; //czerwone
    setLed(mask);
    delay(300);
    mask = (unsigned long)256 << i; //zielone
    setLed(mask);
    delay(300);
    mask = (unsigned long)65536 << i; //niebieski
    setLed(mask);
  delay(300);
